...
# Copyright (c) 2022-2024 The Vault Project
...
# target, so we don't spend time building Vault binaries.
...
GIT_ARCHIVE="${DIST_ARCHIVE_BASE}/vault-source-${VERSION}.tar.gz"
...
if [ ! -e "$GIT_ARCHIVE" ]; then
    ...
    | sort \
    | tar --create --transform "s,^,vault-source-${VERSION}/," --mode='u+rw,go+r-w,a+X' --files-from=- \
    | gzip -9n > ${GIT_ARCHIVE}
...
(
    ...
    # Setup the directory where our Vault build for HOST will be
