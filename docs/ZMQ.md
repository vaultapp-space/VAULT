# The Current/Future Status of ZMQ in Vault

## ZMQ Pub/Sub
Client `ZMQ_SUB` sockets must "subscribe" to topics before it receives any data.
This allows filtering on the server side, so network traffic is reduced. Vault
...
matching supported by both Vault and ZMQ. The `format`, `context` and `event`
...
The Vault daemon will ensure that events prefixed by `chain` will be sent in
...
chain instead of the last block. The Vault daemon also ensures that
...
The Vault daemon will send a `txpool_add` pub exactly once for each
