...
Core tests take longer than any other Vault tests, due to the high amount of computational work involved in validating core components.
...
To run only Vault's core tests (after building):
...
To run only Vault's crypto tests (after building):
...
```bash
vaultd --regtest --offline --fixed-difficulty 1
```
...
Open the wallet file with `vault-wallet-rpc` with RPC port 18083. Finally, start tests by invoking ./blockchain.py or ./speed.py
...
Vault is integrated into [OSS-Fuzz](https://github.com/google/oss-fuzz) and the project integration
is available [here](https://github.com/google/oss-fuzz/tree/master/projects/vault). OSS-Fuzz builds
and runs the fuzzers continuously, so long as Vault's OSS-Fuzz [build script](https://github.com/google/oss-fuzz/blob/master/projects/vault/build.sh) builds them.

Issues found by OSS-Fuzz are publicly available (following a disclosure deadline) on the OSS-Fuzz issue tracker [here](https://issues.oss-fuzz.com/issues?q=project%3Dvault).
The issue tracker only displays limited information, and only maintainers with emails listed in the [project.yaml](https://github.com/google/oss-fuzz/blob/master/projects/vault/project.yaml) have access to full details.

Coverage reports are built on a daily basis and data about this can be found at [introspector.oss-fuzz.com](https://introspector.oss-fuzz.com) [here](https://introspector.oss-fuzz.com/project-profile?project=vault).

### Build and run fuzzers by way of OSS-Fuzz

**Building Vault's fuzzers with OSS-Fuzz**

```sh
$ git clone https://github.com/google/oss-fuzz
$ cd oss-fuzz
$ python3 infra/helper.py build_fuzzers vault
...
```sh
$ python3 infra/helper.py run_fuzzer vault base58_fuzz_tests
...
...
...
To run only Vault's hash tests (after building):
...
To run only Vault's performance tests (after building):
...
To run only Vault's unit tests (after building):
