...
# a main offline vaultd, does most of the tests
# two local online vaultds connected to each other
N_MONERODS = 5

# 4 wallets connected to the main offline vaultd
# 1 wallet connected to the first local online vaultd
# 1 offline wallet
N_WALLETS = 7
...
vaultd_base = [builddir + "/bin/vaultd", "--regtest", "--fixed-difficulty", str(DIFFICULTY), "--p2p-bind-port", "vaultd_p2p_port", "--rpc-bind-port", "vaultd_rpc_port", "--zmq-rpc-bind-port", "vaultd_zmq_port", "--zmq-pub", "vaultd_zmq_pub", "--non-interactive", "--disable-dns-checkpoints", "--check-updates", "disabled", "--rpc-ssl", "disabled", "--data-dir", "vaultd_data_dir", "--log-level", "1", "--rpc-max-connections-per-private-ip", "100", "--rpc-max-connections", "100"]
...
wallet_base = [builddir + "/bin/vault-wallet-rpc", "--wallet-dir", WALLET_DIRECTORY, "--rpc-bind-port", "wallet_port", "--rpc-ssl", "disabled", "--daemon-ssl", "disabled", "--log-level", "1", "--allow-mismatched-daemon-version"]
wallet_extra = [
...
for i in range(N_MONERODS):
  command_lines.append([str(18180+i) if x == "vaultd_rpc_port" else str(18280+i) if x == "vaultd_p2p_port" else str(18380+i) if x == "vaultd_zmq_port" else "tcp://127.0.0.1:" + str(18480+i) if x == "vaultd_zmq_pub" else builddir + "/functional-tests-directory/vaultd" + str(i) if x == "vaultd_data_dir" else x for x in vaultd_base])
  if i < len(vaultd_extra):
    command_lines[-1] += vaultd_extra[i]
  outputs.append(open(FUNCTIONAL_TESTS_DIRECTORY + '/vaultd' + str(i) + '.log', 'a+'))
